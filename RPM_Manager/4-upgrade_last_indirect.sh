...
# @Last Modified time: 2023-04-28 14:52:08
#
    ...
    while read -r pkg_name; do
        if [[ $(yum provides "$pkg_name" --repo="${original_ver}_update" | grep "x86\|arch" | awk '{print $1}' | wc -l) -eq 0 ]]; then
            ...
            if rpm -qa "$pkg_name" | grep "$pkg_name"; then
                # 最终版本
                if [ "$num_pkg_name" -eq 1 ]; then
                    echo "$pkg_name" >>no_mid-ver_update_list
                    echo "There is no last-version of the $pkg_name."
                else
                    mid_pkg_name=$(grep -v "$(rpm -qa "$pkg_name" | awk -F '-' '{printf $3"-"$4}')" /tmp/"${pkg_name}"_list | tail -n 1)
                    yum upgrade -y "$mid_pkg_name" >>pre_update_log
                fi
            else
                # 首次安装的软件包
                mid_pkg_name=$(tail -n 1 /tmp/"${pkg_name}"_list)
                yum upgrade -y "$mid_pkg_name" >>pre_update_log
            fi
        fi
    done <update_list
    ...
    while read -r EPOL_pkg_name; do
        if [[ $(yum provides "$EPOL_pkg_name" --repo="${original_ver}_EPOL_update" | grep "x86\|arch" | awk '{print $1}' | wc -l) -eq 0 ]]; then
            ...
            if rpm -qa "$EPOL_pkg_name" | grep "$EPOL_pkg_name"; then
                # 最终版本
                if [ "$num_EPOL_pkg_name" -eq 1 ]; then
                    echo "$EPOL_pkg_name" >>no_mid-ver_EPOL_update_list
                    echo "There is no last-version of the $EPOL_pkg_name."
                else
                    mid_EPOL_pkg_name=$(grep -v "$(rpm -qa "$EPOL_pkg_name" | awk -F '-' '{printf $3"-"$4}')" /tmp/"${EPOL_pkg_name}"_list | tail -n 1)
                    yum upgrade -y "$mid_EPOL_pkg_name" >>pre_EPOL_update_log
                fi
            else
                # 首次安装的软件包
                mid_EPOL_pkg_name=$(tail -n 1 /tmp/"${EPOL_pkg_name}"_list)
                yum upgrade -y "$mid_EPOL_pkg_name" >>pre_EPOL_update_log
            fi
        fi
