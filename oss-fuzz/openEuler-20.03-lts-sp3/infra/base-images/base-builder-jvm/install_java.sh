...
cd $SRC/
git clone --depth=1 https://ghproxy.com/https://github.com/CodeIntelligenceTesting/jazzer && \
cd jazzer
bazel build --distdir /src --java_runtime_version=local_jdk_15 -c opt --cxxopt="-stdlib=libc++" --linkopt=-lc++ \
