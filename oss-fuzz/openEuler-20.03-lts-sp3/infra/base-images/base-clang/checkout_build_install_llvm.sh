...
LLVM_DEP_PACKAGES="gcc gcc-c++ kernel-devel make cmake ninja-build git python3 libstdc++-devel binutils-devel zlib-devel glibc-devel libgcc libstdc++-devel ncurses-devel"
yum clean all && yum makecache && yum update -y --nogpgcheck && yum install -y $LLVM_DEP_PACKAGES --nogpgcheck
dnf install -y python3-distutils-extra

INTROSPECTOR_DEP_PACKAGES="texinfo bison flex"
if [ -n "$INTROSPECTOR_PATCHES" ]; then
  yum install -y $INTROSPECTOR_DEP_PACKAGES
fi
...
git clone https://github.com/chromium/chromium.git -b main --depth 210000 $SRC/chromium
mkdir $SRC/chromium_tools
cd $SRC/chromium
git checkout 401adcf862beeb83e718b33d499254bd5937b976 
cd ..
# git clone https://chromium.googlesource.com/chromium/src/tools/clang --depth 1
mv $SRC/chromium/tools/clang $SRC/chromium_tools
cd $SRC/chromium_tools/clang
num=`sed -n -e "/with tempfile.TemporaryFile() as f:/=" ./scripts/update.py`
sed -i "${num}i \  filename = url.split('/')[-1]" ./scripts/update.py
sed -i "`expr $num + 1`i \  print(os.path.join('/src',filename))" ./scripts/update.py
sed -i "`expr $num + 2`i \  if os.path.isfile(os.path.join('/src',filename)):" ./scripts/update.py
sed -i "`expr $num + 3`i \    t = tarfile.open(os.path.join('/src',filename), 'r:*')" ./scripts/update.py
sed -i "`expr $num + 4`i \    members = None" ./scripts/update.py
sed -i "`expr $num + 5`i \    if path_prefixes is not None:" ./scripts/update.py
sed -i "`expr $num + 6`i \      members = [m for m in t.getmembers()" ./scripts/update.py
sed -i "`expr $num + 7`i \        if any(m.name.startswith(p) for p in path_prefixes)]" ./scripts/update.py
sed -i "`expr $num + 8`i \    t.extractall(path=output_dir, members=members)" ./scripts/update.py
sed -i "`expr $num + 9`i \  else:" ./scripts/update.py
num=`sed -n -e "/with tempfile.TemporaryFile() as f:/=" ./scripts/update.py`
sed -i "${num},+14s/^/  /" ./scripts/update.py
rm -rf $SRC/chromium
...
# 已存放到本地，缩短测试耗时。交付时取消下行注释，并注释 llvm-project 的复制行即可。
clone_with_retries https://gitee.com/mirrors/llvm-project.git $LLVM_SRC
...
# yum remove -y $LLVM_DEP_PACKAGES
if [ -n "$INTROSPECTOR_PATCHES" ]; then
  yum remove -y $INTROSPECTOR_DEP_PACKAGES
fi
