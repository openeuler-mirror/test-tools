...
deploy_tag_file_bak='/root/installed_components_bak'
...
function install_select_component() {
    ...
    deploy_component_chain ${component_chain}
    cp ${deploy_tag_file} ${deploy_tag_file_bak}
    echo  hadoop > ${deploy_tag_file}
    echo hbase >> ${deploy_tag_file}
...
function uninstall_all_component() {
  ...
  read -r -a component_list<<<$(cat ${deploy_tag_file_bak} | awk '{print $1}' | xargs)
  n=1
  num=${#component_list[*]}
  while [ ${n} -le ${num} ]; do
...
EOF
      sed -i "/^${comp}.*/d" ${deploy_tag_file_bak}
      let "n++"
  done
...
if [ $# -eq 3 ]; then
    if [ "install_select" = $1 ] && [ "true" = $3 ]; then
        ...
        echo -e "\033[1;4;4;33m[Usage1] 安装完成 \033[0m"
    else 
