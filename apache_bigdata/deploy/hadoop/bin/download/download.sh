...
mkdir -p "${tool_root_dir}"/deps/"${platform}"/"${hadoop_version}"/ > /dev/null 2>&1
# Download hadoop
if ! wget -O "${tool_root_dir}"/deps/"${platform}"/"${hadoop_version}"/"${package}" "${download_url}"/tarball/"${arch}"/"${os}"/"${platform}"/"${package}"; then
