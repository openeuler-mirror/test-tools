...
data_file = os.path.join(conf.get_data_path(), "aops-apollo", "get_task_progress.yaml")
log = my_log()
...
class TestTaskProgress:
    ...
    def setup_class():
        ...
        ApiApollo().execute_task({"task_id": Yaml(conf.get_common_yaml_path()).data()['task_id']})
        ApiApollo().execute_task({"task_id": Yaml(conf.get_common_yaml_path()).data()['hot_fix_way_task_id']})
        ApiApollo().execute_task({"task_id": Yaml(conf.get_common_yaml_path()).data()['cold_fix_way_task_id']})
    ...
    def test_task_progress(self, test_data):
        ...
        if res["body"]["code"] == '200':
            assert_res.assert_data(res["body"]["data"]["result"], test_data["validate"]["data"]["result"])
