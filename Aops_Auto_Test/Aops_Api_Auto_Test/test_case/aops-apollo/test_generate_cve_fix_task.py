...
data_file = os.path.join(conf.get_data_path(), "aops-apollo", "task_cve-fix_generate.yaml")
log = my_log()


class TestGenerateCveFixTask:
    ...
    def teardown_class():
        log.info("清理当前测试套数据")
        QueryDataBase().delete_host(Yaml(conf.get_common_yaml_path()).data()['host_name'])
    ...
    def test_generate_cve_fix_task(self, test_data):
        ...
        if res["body"]["code"] == '200' and test_data["validate"]["data"]:
            assert_res.assert_data(res["body"]["data"], test_data["validate"]["data"])
            assert_res.assert_database(test_data["validate"]['sql'], before_value+len(test_data["validate"]["data"]))
