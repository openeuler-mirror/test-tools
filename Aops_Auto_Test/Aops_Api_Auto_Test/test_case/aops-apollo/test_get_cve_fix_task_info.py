...
data_file = os.path.join(conf.get_data_path(), "aops-apollo", "get_cve_fix_task_info.yaml")
log = my_log()
CreateData().generate_cve_fix_task()


class TestGetCveFixTaskInfo:
    ...
    def test_get_cve_fix_task_info(self, test_data):
        log.info("test_data: {}".format(test_data))
        data = test_data["data"]
        res = ApiApollo().get_cve_fix_task_info(data)
