...
data_file = os.path.join(conf.get_data_path(), "aops-apollo", "query_host_list.yaml")
log = my_log()
CreateData().get_host_info()


class TestQueryHostList:
    ...
    def test_query_host_list(self, test_data):
        log.info("test_data: {}".format(test_data))
        data = test_data["data"]
        res = ApiApollo().query_host_list(data)
        ...
        if res["body"]["code"] == 200:
            assert_res.assert_search(res["body"]["data"]['result'], test_data["validate"]["data"]['result'])
