...
data_file = os.path.join(conf.get_data_path(), "aops-diana", "create_workflow.yaml")
log = my_log()
...
class TestCreateWorkflow:
    ...
    def teardown_class():
        log.info("清理当前测试套数据")
        ApiDiana().delete_workflow({"workflow_id": workflow_id})
    ...
    def test_create_workflow(self, test_data):
        ...
        if res["body"]["code"] == "200":
            global workflow_id
            workflow_id = res["body"]["data"]["workflow_id"]
...
if __name__ == '__main__':
    pytest.main(["-s", "test_create_workflow.py", "--alluredir", "./report/result"])