...
yum remove -y expect nfs-utils-help
yum install -y tar python2
yum remove -y mkpasswd
# snappy adapter
rpm -e snappy --nodeps
...
rpm -qa|grep redhat-rpm-config-182-1.fc34 &>/dev/null
if [ $? -ne  0 ];then
    flag=1
    echo "redhat-rpm-config-182-1.fc34 install failed"
fi
pip2 -V &>/dev/null
if [ $? -ne  0 ];then
    flag=1
